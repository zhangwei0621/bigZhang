package com.study.googletranslatedemo.demo.editor.effect.splash.custom
...
class FilterPaintImageView @JvmOverloads constructor(
    ...
    // 滤镜画笔强度
    var filterPaintIntensity: Int = 255
        set(value) {
            field = value
    ...
    // 恢复画笔强度
    var restorePaintIntensity: Int = 255
        set(value) {
            field = value
    ...
    var filter: String = FilterRule.BLACK
    ...
    override fun setImageBitmap(bm: Bitmap?) {
        // TODO: 这里用drawable判断逻辑会更加通用
        if (bm != null) {
            // 生成滤镜处理图像和着色器
            val tileMode = Shader.TileMode.CLAMP
            _restoreShader = BitmapShader(bm, tileMode, tileMode)
            val filterBitmap =
                CGENativeLibrary.filterImage_MultipleEffects(bm, filter, 1f)
